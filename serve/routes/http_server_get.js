...
router.post('/insert_img', async (ctx) => {
  ...
  //推断情况判断
  if(!confidence&&!person){
    //插入两次原图以结束路由内容
    //可信度转化函数
    function tolittle(value){
    ...
    if (!image || !Array.isArray(image) || image.length !== 2) {
      return ctx.status = 400, ctx.body = { error: '需要上传2个图片资源' };
    }
    ...
    try {
      ...
      const matches1 = image[0].match(/^data:image\/(jpeg|png|jpg);base64,(.+)$/);
      if (!matches1) {
        return ctx.status = 400, ctx.body = { error: '第一个图片的Base64编码无效' };
      }
      ...
      const matches2 = image[1].match(/^data:image\/(jpeg|png|jpg);base64,(.+)$/);
      if (!matches2) {
        return ctx.status = 400, ctx.body = { error: '第二个图片的Base64编码无效' };
      }
      ...
      // 保存文件信息到数据库
      console.log("插入前的person取值:"+person);
      await connection1.execute(`
        INSERT INTO t_behavior_data(d_no, field1, field2, c_time, is_saved, file_type , result, confidence)
        VALUES ('${d_no}', '${fileName1}', '${fileName2}', '${formattedTime}', "实时数据", "picture",'${(person===0?'1':'0')}','1')
      `);
      ...
      ctx.status = 500;
      ctx.body = { error: '保存文件失败: ' + error.message };
    }
  }else{
    //可信度转化函数
    function tolittle(value){
    ...
    try {
      ...
      await connection1.execute(`
        INSERT INTO t_behavior_data(d_no, field1, field2, c_time, is_saved, file_type , result, confidence)
        VALUES ('${d_no}', '${fileName1}', '${fileName2}', '${formattedTime}', "实时数据", "picture",'${(person === 0 ? '1' : '0')}','${tolittle(confidence)}')
      `);
