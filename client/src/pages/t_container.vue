  ...
  <div class="content1" v-show="Pinia.Device_sign">
    ...
    <div class="top_part">
      ...
      <div class="block1 block" v-show="Pinia.Device_sign">
        <el-date-picker v-model="value1" @change="submit" type="datetimerange" start-placeholder="Start Date"
...
<script setup lang="ts">
  ...
  let result_all;
  (async function () {
    result_all = await axios.get(`/api/t_device/first`);//获取到总内容用于type_array进行更新
    // 主动调用一次update1
    update();
  })()
  ...
  onMounted(async () => {
    ...
    //用于历史数据查询方法的赋值
    submit = async () => {
      const result = await axios.get(`/api/History/container?start=${value1.value[0]}&end=${value1.value[1]}`);
      device_array.value = result.data;
      xuanran(device_array.value);
    }
  });
