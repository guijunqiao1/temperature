...
Router_direct.get("/zhiling", async (ctx) => {
  const { mode, d_no } = ctx.request.query;
  const result = await connection.execute(`
    SELECT c.id, c.f_type, c.mode, c.f_value, c.t_name, c.max, c.min, c.topic, c.header, c.luyou, t.value, c.label_boolean, c.label, t.d_no
    FROM t_direct t, t_direct_config c
    WHERE c.id = t.config_id
  ...
  rowx.forEach((item, index) => {
    if (item.value.split(" ")[1]) {
      // 若为时间值
    } else if (Number.isNaN(Number(item.value))) {
      // 若为中文值
    } else {
      item.value = Number(item.value);
    }
...
Router_direct.get("/label_boolean", async (ctx) => {
  console.log("成功进入路由");
  let { content, id } = ctx.request.query;
  id = Number(id);
  console.log("TYPE:id" + typeof id);
...
// 获取控件中机房总类型数量的路由
Router_direct.get("/type_device", async (ctx) => {
  const result = await connection.execute(`
    SELECT d_no
    FROM t_direct 
    WHERE config_id = 12
...
// 机房历史记录查询路由
Router_direct.get("/operate_history", async (ctx) => {
  const { d_no } = ctx.request.query;
  const [row] = await connection.execute(`
    SELECT *
    FROM operate_history
    WHERE place = '${d_no}'
